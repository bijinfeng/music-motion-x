...
class FontPlayer {
  ...
  _findcurFontNum(curTime, startIndex = 0) {
    ...
    for (let index = startIndex; index < length; index++)
      if (curTime < this.fonts[index].startTime)
  ...
  _refresh() {
    ...
    if (driftTime >= 0 || this.curFontNum == 0) {
      ...
      if (this.delay > 0) {
        ...
        let newCurLineNum = this._findcurFontNum(
          currentTime,
          this.curFontNum + 1
        );
        if (newCurLineNum > this.curFontNum)
          this.curFontNum = newCurLineNum - 1;
